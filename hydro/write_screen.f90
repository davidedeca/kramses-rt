subroutine write_screen
  ...
  real(dp)::dx,scale,smallp,ddd,ppp,sss

  integer     ,dimension(:),allocatable::ind_grid,ind_cell,ind_sort,ll,ll_all
  real(qdp),dimension(:),allocatable::rr,et,ei,dd,uu,mm,gg,dtot,ss
  real(qdp),dimension(:),allocatable::rr_all,et_all,ei_all
  real(qdp),dimension(:),allocatable::dd_all,uu_all,mm_all,gg_all,dtot_all,ss_all
#if NENER>0
  integer::irad
  ...
  allocate(rr(1:ncell),mm(1:ncell),dd(1:ncell),dtot(1:ncell),ss(1:ncell))
  allocate(et(1:ncell),ei(1:ncell))
  allocate(uu(1:ncell),ll(1:ncell),gg(1:ncell))
  allocate(rr_all(1:ncell),mm_all(1:ncell),dd_all(1:ncell),dtot_all(1:ncell),ss_all(1:ncell))
  ...
  ei=0.0D0; uu=0.0D0; gg=0.0D0; ll=0; ss=0.0D0
  rr_all=0.0D0; mm_all=0.0D0; dd_all=0.0D0; dtot_all=0.0D0; et_all=0.0D0
  ei_all=0.0D0; uu_all=0.0D0; gg_all=0.0D0; ll_all=0; ss_all=0.0D0
#if NENER>0
           ...
           do ind=1,twotondim
              ...
              do i=1,ncache
                 if(son(ind_cell(i))==0)then
                    ...
                    ss(icell)=uold(ind_cell(i),4)/dd(icell)
                    mm(icell)=dd(icell)
  ...
  call MPI_ALLREDUCE(ss,ss_all,ncell,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
  ...
  ei=ei_all; uu=uu_all; gg=gg_all; ll=ll_all; ss=ss_all
#if NENER>0
  ...
  if(myid==1)then
     ...
     if(poisson)then
        ...
        write(*,*)'lev      x           d          u          P        passive_scalar'
     endif
#endif
     ! Sort radius
  ...
  deallocate(mm,rr,dd,dtot,et,ei,uu,ll,gg,ss)
  deallocate(mm_all,rr_all,dd_all,dtot_all,et_all,ei_all,uu_all,ll_all,gg_all,ss_all)
#if NENER>0
...
113 format(i3,1x,1pe12.5,1x,9(1pe12.5,1x))

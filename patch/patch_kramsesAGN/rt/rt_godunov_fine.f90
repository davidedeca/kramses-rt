...
!-------------------------------------------------------------------------
  ...
  use rt_parameters
  implicit none
  integer::ilevel,ncache,ind
  real(dp)::dt, dx_loc, ix,iy,iz
  ...
  real(dp),dimension(1:nvector,1:ndim)::xxx
...
!------------------------------------------------------------------------
  ...
  do ind=1,twotondim
     do idim=1,ndim
         do i=1,ncache
             xxx(i,idim)=xg(ind_grid(i),idim)+xc(ind,idim)
        end do
     end do
  ! Rescale position from code units to user units
     do idim=1,ndim
         do i=1,ncache
             xxx(i,idim)=(xxx(i,idim)-skip_loc(idim))*scale
         end do
     end do
  end do
  ...
  do idim=1,ndim
     ...
     do i2=i2min,i2max      ! i.e. update one grid at level ilevel
        ...
        do ivar=1,nrtvar
           do i=1,ncache
              !dx_loc=boxlen/2**icoarse_max
              dx_loc = boxlen/2**levelmin
              factor = ((d_init + xxx(i,1) - dx_loc)/(d_init + xxx(i,1)))**2
              !if (ivar==1) print*, 'A', rtunew(ind_cell(i),ivar)*scale_Np/rt_c
              rtunew(ind_cell(i),ivar)=                 &
                   &  rtunew(ind_cell(i),ivar)          &
                   & +(flux(i,i3   ,j3   ,k3   ,ivar,idim)*factor  &
                   & - flux(i,i3+i0,j3+j0,k3+k0,ivar,idim))
              !if (ivar==1) print*, 'B', rtunew(ind_cell(i), ivar)*scale_Np/rt_c
           end do
        end do
     end do
     end do

...
!-------------------------------------------------------------------------
  namelist/rt_params/rt_star, rt_esc_frac, rt_flux_scheme, rt_smooth     &
       ...
       & ,rt_err_grad_xHI, rt_floor_xHI, rt_err_grad_xH2, rt_floor_xH2   &
       &, rt_refine_aexp                  &
       & ,is_mu_H2,rt_isIR, is_kIR_T, rt_T_rad, rt_vc, rt_pressBoost     &
       & ,rt_isoPress, rt_isIRtrap, rt_movie_vars,d_init, v_outflow      &
...
101 continue                                   ! No harm if no rt namelist
  ...
  if(rt_err_grad_n .gt. 0. .or. rt_err_grad_xHII .gt. 0.                 &
       .or. rt_err_grad_xHI .gt. 0. .or. rt_err_grad_xH2 .gt. 0.) rt_refine=.true.
...
!-------------------------------------------------------------------------
  ...
  use SED_module
  implicit none
  real(dp), dimension(13)::groupL0_default, groupL1_default, group_egy_default
  integer::i
!-------------------------------------------------------------------------
  ...
  groupL0_default   =  (/0.7542d0, 2.65d0, 6.0d0 ,  11.2d0,  13.6d0  , 14.159d0, 15.4d0 ,  24.59d0, 30.0d0 ,  54.42d0, 1d2, 2d2, 1d3/)
  groupL1_default   =  (/2.65d0  , 6.0d0 , 11.2d0,  13.6d0,  14.159d0, 15.4d0  , 24.59d0,  30.0d0 , 54.42d0,  1d2, 2d2, 1d3, 1d4/)
  group_egy_default =  (/1.96300977623 , 4.48983275063 , 8.37722424786 , 12.3017463205 ,13.8734719772 , 14.7482029986 , 18.1673223329 , 26.5186427192 , 32.7650226189 , 61., 150., 500., 5000./)

  groupL0(1:min(nGroups, 13)) = groupL0_default(1:min(nGroups, 13))
  groupL1(1:min(nGroups, 13)) = groupL1_default(1:min(nGroups, 13))
  group_egy(1:min(nGroups, 13)) = group_egy_default(1:min(nGroups, 13))

  ! DEFINE IONIZATION ENERGIES
  ionEvs(1) = 13.60d0   ! H
  ionEvs(2) = 0.0d0     ! E
  ionEvs(3) = 0.0d0     ! H+
  ionEvs(4) = 24.59d0   ! HE
...
!------------------------------------------------------------------------
  ...
  real(dp)::vol,r,xn,yn,zn,en,rt_n_source_rescaled,distance,time1
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
...
!------------------------------------------------------------------------
  ...
  distance = d_init + v_outflow * t

  ! Loop over RT regions
  do k=1,rt_nsource

     rt_n_source_rescaled = rt_n_source(k) * (d_init / distance)**2
     ...
     if(rt_source_type(k) .eq. 'square')then
       ! Exponent of choosen norm
        ...
        do i=1,nn
           ...
           if(r<1.0)then
              uu(i,group_ind) = rt_n_source_rescaled/rt_c_cgs/scale_Np
              ! The input flux is the fraction Fp/(c*Np) (Max 1 magnitude)
              uu(i,group_ind+1) = rt_u_source(k) * rt_c * rt_n_source_rescaled / scale_Np
#if NDIM>1
              uu(i,group_ind+2) = rt_v_source(k) * rt_c * rt_n_source(k) / scale_Np 
#endif
#if NDIM>2
              uu(i,group_ind+3) =  rt_w_source(k) * rt_c * rt_n_source(k) / scale_Np
#endif
           end if
        end do
     end if

...
!-------------------------------------------------------------------------
  ...
  real(dp),dimension(krome_nmols):: dXion
#else
...
#if defined(krome_on)
     dustAbs(:)  = kAbs_loc(:) *rho*dust_for_krome*rt_c_cgs*krome_get_fevap(dT2,dXion)
     dustSc(iIR) = kSc_loc(iIR)*rho*dust_for_krome*rt_c_cgs*krome_get_fevap(dT2,dXion)
#else
...
# endif
  ...
  !call krome_set_J21xray(get_photoIntensity(1d3)*1.60217646d-12/(4*krome_pi) * 1d21)

recursive subroutine amr_step(ilevel,icount)
...
#endif
                 if(momentum_feedback .and. (.not. static))call make_virtual_fine_dp(pstarold(1),i)
  ...
  if(ilevel==levelmin)then
     ...
     if(mod(nstep_coarse,foutput)==0.or.aexp>=aout(iout).or.t>=tout(iout).or.output_now_all.EQV..true.)then
        ...
        if(myid==1)print*, '-------------entering backup'
        call dump_all
  ...
  if(ilevel==levelmin)then
     ...
     if(hydro.and.star.and.eta_sn>0.and.f_w>0.and.(.not.static))call kinetic_feedback
  ...
  if(hydro.and.star.and.(.not.static_gas)) call count_leaf_cells()
...
#if NDIM==3
                               call timer('feedback','start')
  if(hydro.and.star.and.eta_sn>0.and.(.not.static))call thermal_feedback(ilevel, dtnew(ilevel))
#endif
...
#if NDIM==3
                               call timer('feedback','start')
  if(hydro.and.star.and.(.not.static_gas))call star_formation(ilevel,dtnew(ilevel))
#endif

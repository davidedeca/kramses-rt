...
!-------------------------------------------------------------------------
...
#if defined(krome_on)
  real(dp),dimension(1:krome_nmols, 1:nvector):: xion
  real(dp),dimension(krome_nmols):: dXion, dXion_old
#else
  ...
  real(dp)::dt_rec
  real(dp):: dT2, dT2_old
  real(dp),dimension(nGroups):: dNp, dNpexp
...
contains
  ...
  ! dt      =>  Timestep size [s]
  ! a_exp   =>  Cosmic expansion
  ! dt_ok   <=  .f. if timestep constraints were broke. otherwise
  ...
  !-----------------------------------------------------------------------
    ...
    use krome_user, ONLY: krome_get_fevap
#endif
    implicit none
#if defined(krome_on)
  integer::i_photo_rea,idx
  real(kind=8)::Z_for_krome,dust_for_krome,H2shield
  real(dp),dimension(krome_nPhotoBins)::flux_for_krome
    ...
    real(dp),dimension(nGroups),save:: recRad, phAbs, phSc, dustAbs, H2abs
    ...
    if(dT2<0) then
       print*, 'ALERT: NEGATIVE TEMPERATURE', dT2
       call abort
    endif
    ...
    neInit=ne
#else
  nHe   = dXion(KROME_idx_HEjj) +dXion(KROME_idx_HEj) + dXion(KROME_idx_HE)
  nN(1) = dXion(KROME_idx_H)
  nN(2) = dXion(KROME_idx_HE)
  nN(3) = dXion(KROME_idx_HEj)
  nI(1) = dXion(KROME_idx_Hj)
  nI(2) = nN(3)
  nI(3) = dXion(KROME_idx_HEjj)
  TK    = dT2 ! Temperature is already multiplied for molecular weight
  if(rt_isTconst) TK=rt_Tconst                         !  Force constant T
  ne    = dXion(KROME_idx_E)
  neInit=ne
#endif
...
#if defined(krome_on)
  Z_for_krome = Zsolar(icell)
  Z_for_krome = max(krome_met_low,Z_for_krome)
#if defined(asano)
  dust_for_krome = asano_dust_to_gas(Z_for_krome)
#else
  dust_for_krome = Z_for_krome
#endif
  call krome_set_Z(Z_for_krome)
  call krome_set_dust_to_gas(dust_for_krome)
#endif
#if defined(krome_on)
    rho = mH * ((dXion(KROME_idx_H)+dXion(KROME_idx_Hj)+dXion(KROME_idx_Hk)) &
              + 4*(dXion(KROME_idx_HE)+dXion(KROME_idx_HEj)+dXion(KROME_idx_HEjj)) &
              + 2*(dXion(KROME_idx_H2)+dXion(KROME_idx_H2j)))
#else
    rho = nH(icell) / X * mH
#endif
#if NGROUPS>0
    ...
    if(rt .and. nGroups .gt. 0) then
     ...
     dustAbs(:)  = kAbs_loc(:) *rho*dust_for_krome*rt_c_cgs*krome_get_fevap(dT2)
     dustSc(iIR) = kSc_loc(iIR)*rho*dust_for_krome*rt_c_cgs*krome_get_fevap(dT2)
#else
    ...
    if(rt .and. rt_advect) then
     ...
     do i_bin=1,krome_nPhotoBins
       ...
       do i_photo_rea=1,krome_nPhotoRates
         ...
         phAbs(i_bin) = phAbs(i_bin) + dXion(idx) * photoBinJTab(i_photo_rea,i_bin)
       enddo
       ! rate [s-1]
       if(groupL0(i_bin) .ge. 11.2d0 .and.  groupL1(i_bin) .le. 13.6d0) then
         ! f_shield = exp(-sigma n dx)
         H2shield = max(calc_H2shieldR14_for_ramses(dXion(KROME_idx_H2), dT2, krome_get_user_cell_size()), tiny(1.d0))
     ...
     enddo
       ...
       do igroup=1,nGroups        ! Deplete photons, since they go into IR
          if( .not. (rt_isIR .and. igroup.eq.iIR) ) then  ! IR done elsewhere
...
!       if (dNp(4) > 1.e-20) then
!        !print*, 'unabsorbed:', dNp_old(4), ' for cell', icell, ' cpu', myid
!        print*, 'A', dNp(4), dXion(KROME_idx_H2), icell, myid
!       endif
       ...
       do igroup=1,nGroups ! -------Momentum transfer from photons to gas:
          ...
          !if(mom_fact*dFp(1,igroup) > 1.e-18)then
          !  print*, '---', igroup, ddt(icell), phAbs(igroup), exp(-ddt(icell) * (phAbs(igroup) + phSc(igroup)))
          !  print*, '     ', dFp(1,igroup), one_over_c_cgs, one_over_rt_c_cgs
          !  print*, '     ', group_egy_erg(igroup), mom_fact*dFp(1,igroup)
          !endif
    ...
    if(rt_isIR) then
#if defined(krome_on)
        print*, 'Aborting, krome does not update IR radiation at the moment'
        call clean_stop
# endif
...
#else
  !if (dNp(4) .ge. 1) print*, dNp
  ! [Np] = cm-3
  flux_for_krome(1:krome_nPhotoBins) = dNp(1:krome_nPhotoBins)
  ! eV/cm3
  ...
  !real(dp)::krome_rt_scale
  !call krome_rt_units(krome_rt_scale)
  ! from cm-3 to ev/cm2/s/Hz/sr
  !flux_for_krome(1:krome_nPhotoBins) = krome_rt_scale*dNp(1:krome_nPhotoBins) * krome_get_photoBinE_mid()/krome_get_photoBinE_delta()
  ...
  !if (dXion(1)<1d-10) then
  !   print*, 'before krome - n problem' ,dT2, dXion(:)
  !   call abort
  !endif
!
!  if (dT2 > 1e9 .or. dT2 < 1. .or. isnan(dT2)) then
 !    print*, 'before krome - T problem' ,dT2, dXion(:)
 !    call abort
 ! endif
  ...
  call krome(dXion(:), dT2, dt)

  if (dT2 < 10.) then
           print*, dT2, 'after krome call'
           call abort
  endif
  ...
  do iii=1, 9 
  if (dXion(iii)==0.0)then
      print*, 'after krome', dXion(iii), dT2
  endif
  enddo

  !if ((dT2_old > 1e6) .and. (dT2 < dT2_old)) then
  !    dXion = dXion_old
  !    dT2   = dT2_old
  !endif
  ...
  !if (dT2 > 1e9 .or. dT2 < 1. .or. isnan(dT2)) then
  !   print*, 'after krome - T problem', dT2, dXion(:)
  !   call abort
  !endif

  !if (dXion(1)<1d-10) then
  !   print*, 'after krome - n problem' ,dT2, dXion(:)
  !   call abort
  !endif

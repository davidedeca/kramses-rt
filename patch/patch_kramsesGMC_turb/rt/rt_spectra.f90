...
!_________________________________________________________________________
  ...
  real(dp),allocatable,dimension(:)::SNtimes
...
!-------------------------------------------------------------------------
  ...
  real(kind=8),allocatable::Zs(:), Ls(:), rebAges(:), ages(:)
  real(kind=8),allocatable::SEDs(:,:,:)           ! SEDs f(lambda,age,met)
  real(kind=8),allocatable::tbl(:,:,:), reb_tbl(:,:,:)
  integer::i,ia,iz,ip,ii
  real::dum
  character(len=128)::fZs, fAges, fSEDs, fSN    ! Filenames
  logical::ok,okAge,okZ,okSN
...
!-------------------------------------------------------------------------
  ...
  write(fSN, '(a,a)')  trim(sed_dir),"/star_list.dat"
  inquire(file=fZs, exist=okZ)
  inquire(file=fAges, exist=okAge)
  inquire(file=fSEDs, exist=ok)
  inquire(file=fSN,exist=okSN)
  if(.not. ok .or. .not. okAge .or. .not. okZ .or. .not. okSN) then
     if(myid.eq.1) then
        write(*,*) 'Cannot read SED files...'
        write(*,*) 'Check if SED-directory contains the files ',  &
                   'metallicity_bins.dat, age_bins.dat, all_seds.dat,star_list.dat'
     endif
     call clean_stop
  end if
  ...
  allocate(ages(nAges))
  do i = 1, nAges
     read(10,'(e14.6)') ages(i)    ! kramsesGMC : ages is used for star id !!!
  end do
  close(10)
  ...
  allocate(Ls(nLs))
  read(10, *) Ls(:)
  allocate(SED_table(nAges,nzs,nLs,1))
  do iz = 1, nzs
     do ia = 1, nAges
        read(10,*) SED_table(ia,iz,:,1)
     end do
  end do
  close(10)
  ! READ SUPERNOVA EXPLOSION TIMES----------------------------------------
  open(unit=10,file=fSN,status='old',form='formatted')
  allocate(SNtimes(nAges))
  do i = 1, nAges
     read(10, *) dum, dum, dum, SNtimes(i)
  end do
  close(10)
...
!-------------------------------------------------------------------------
  if(rt_star.and. nstar_tot .gt. 0)rt_advect=.true.
  return  ! DO NOT UPDATE SED
...
!  endif
  ...
  ret = SED_table(INT(age1_Gyr), 1, :, 1) * dt_Gyr * 1.e9 * 3.1536e7
...
!-------------------------------------------------------------------------
  ...
  do j = 1, np
     ...
     !print*,'qui', j,ind_part(j)
     !flush(5)
     !flush(6)
     ! Possibilities:     Born i) before dt, ii) within dt, iii) after dt:
     dt_loc_Gyr = max(min(dt_Gyr, age), 0.)
     !print *,j,z!,ind_part(j)
     !print *,idp(ind_part(j)),dt_loc_Gyr,z,part_NpInp(j,1:nSEDgroups)
     !flush(5)
     if(age*1.e3 .lt. SNtimes(ind_part(j)))then
        call getNPhotonsEmitted(DBLE(idp(ind_part(j))),dt_loc_Gyr,z,part_NpInp(j,1:nSEDgroups))
     else
        part_NpInp(j,1:nSEDgroups) = 0
     endif
     mass = mp(ind_part(j))
     ...
     part_NpInp(j,:) = part_NpInp(j,:)*rt_esc_frac / scale_l**3 / scale_np / vol_loc   ! 
  ...
  do j=1,np
     if( ok(j) ) then                                      !   ilevel cell
        do ip=1,nSEDgroups
           ...
           !print*, indp(j), iGroups(ip), part_npinp(j,ip) ,rtunew(indp(j), iGroups(ip))
        end do
     else                                                  ! ilevel-1 cell
        do ip=1,nSEDgroups

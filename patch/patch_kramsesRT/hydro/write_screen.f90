subroutine write_screen
  ...
  real(dp)::dx,scale,smallp,ddd,ppp,sss1, sss2, sss3
  character(LEN=5)::nchar
  character(LEN=80)::filename
  integer,dimension(:),allocatable::ind_grid,ind_cell,ind_sort,ll,ll_all
  real(qdp),dimension(:),allocatable::rr,et,ei,dd,uu,mm,gg,dtot,ss1, ss2, ss3
  real(qdp),dimension(:),allocatable::rr_all,et_all,ei_all
  real(qdp),dimension(:),allocatable::dd_all,uu_all,mm_all,gg_all,dtot_all,ss1_all, ss2_all, ss3_all
#if NENER>0
  integer::irad
  ...
  allocate(rr(1:ncell),mm(1:ncell),dd(1:ncell),dtot(1:ncell),ss1(1:ncell),ss2(1:ncell),ss3(1:ncell))
  allocate(et(1:ncell),ei(1:ncell))
  allocate(uu(1:ncell),ll(1:ncell),gg(1:ncell))
  allocate(rr_all(1:ncell),mm_all(1:ncell),dd_all(1:ncell),dtot_all(1:ncell),ss1_all(1:ncell),ss2_all(1:ncell),ss3_all(1:ncell))
  ...
  ei=0.0D0; uu=0.0D0; gg=0.0D0; ll=0; ss1=0.0D0; ss2=0.0D0; ss3=0.0D0
  rr_all=0.0D0; mm_all=0.0D0; dd_all=0.0D0; dtot_all=0.0D0; et_all=0.0D0
  ei_all=0.0D0; uu_all=0.0D0; gg_all=0.0D0; ll_all=0; ss1_all=0.0D0; ss2_all=0.0D0; ss3_all=0.0D0
#if NENER>0
           ...
           do ind=1,twotondim
              ...
              do i=1,ncache
                 if(son(ind_cell(i))==0)then
                    ...
                    ss2(icell)=uold(ind_cell(i),11+NENER)/dd(icell)
...
#if defined(RT)
     if(poisson)then
        write(*,*)'lev, x, d, u, P, H, H2, HII, u1, u2, u3, u4, u5, u6, u7, u8, u9, u10'
     else
        write(*,*)'lev, x ,d ,u, P, H, H2, HII, u1, u2, u3, u4, u5, u6, u7, u8, u9, u10'
     endif
#endif
  ...
  deallocate(mm,rr,dd,dtot,et,ei,uu,ll,gg,ss1, ss2, ss3)
  deallocate(mm_all,rr_all,dd_all,dtot_all,et_all,ei_all,uu_all,ll_all,gg_all,ss1_all, ss2_all, ss3_all)
#if NENER>0
...
113 format(i3,1x,1pe12.5,1x,17(1pe12.5,1x))

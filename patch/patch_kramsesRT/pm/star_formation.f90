  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        if(sf_virial)then
           ...
           ! FORCE FORMATION OF ONLY 1 STAR PARTICLE AT THE CENTER OF THE GRID
           if (dtold(levelmin)==0) then
            do i=1,ngrid
               x=(xg(ind_grid(i),1)+xc(ind,1)-skip_loc(1))
               y=(xg(ind_grid(i),2)+xc(ind,2)-skip_loc(2))
               z=(xg(ind_grid(i),3)+xc(ind,3)-skip_loc(3))
               if (x==0.50781250000000000 .and. y==0.50781250000000000 .and. z==0.50781250000000000) then
                 ok(i)=.true.               
                 print*, 'NEW STAR AT POSITION:', x, y, x
               else                 
                 ok(i)=.false.
               endif
            enddo
        ...
        do i=1,ngrid
           nstar(i)=0
           if(ok(i))then
              ! ONLY 1 STAR PARTICLE FORMS
              nstar(i)=1 
              mstar = 1e4 * 2.e33 / scale_d / scale_l**3           

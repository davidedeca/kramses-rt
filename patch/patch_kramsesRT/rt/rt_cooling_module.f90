...
#if defined(krome_on)
     dustAbs(:)  = kAbs_loc(:) *rho*dust_for_krome*rt_c_cgs
     dustSc(iIR) = kSc_loc(iIR)*rho*dust_for_krome*rt_c_cgs
#else
...
!       print*, "u", dNp(4), dXion(KROME_idx_H2),icell, myid
!       dNp_old(4) = dNp(4)
!      endif      
...
!       endif
       ...
       do igroup=1,nGroups ! -------Momentum transfer from photons to gas:
          mom_fact = exp(-ddt(icell) * (phAbs(igroup) + phSc(igroup))) &

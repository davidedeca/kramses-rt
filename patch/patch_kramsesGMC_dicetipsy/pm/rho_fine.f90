     ...
     do ind=1,twotondim
        ...
        do i=1,active(ilevel)%ngrid
           if(phi(active(ilevel)%igrid(i)+iskip)>=m_refine(ilevel))then
              ...
              !if (ilevel==10)then ! .and. (uold(active(ilevel)%igrid(i)+iskip, 1) < 0.0011))  then 
              !   print*, uold(active(ilevel)%igrid(i)+iskip, 1) , phi(active(ilevel)%igrid(i)+iskip), m_refine(ilevel)
              !endif

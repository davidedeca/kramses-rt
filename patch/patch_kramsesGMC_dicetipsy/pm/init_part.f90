subroutine init_part
  ...
  integer::ipart,jpart,ipart_old,ilevel,idim, ipart_gas, ipart_star
  ...
  real(dp)::dx,xx1,xx2,xx3,vv1,vv2,vv3,mm1,zz1,uu1,hh1,ee1,yy1
  ...
  real(kind=8),dimension(1:nvector)::mm,uu,zz,ee,hh,yy
  type(part_t)::tmppart
...
!=====================================
  ...
  subroutine load_ascii
    ...
    if(TRIM(initfile(levelmin)).NE.' ')then
       filename=TRIM(initfile(levelmin)) !here also gas particles
       if(myid==1)then
       ...
       do while (.not.eof)
          ...
          if(myid==1)then
             jpart=0
             do i=1,nvector
                if (jpart < n_star_part)then  !!wrong, because jpart is per proc
                  ...
                  vv(i,3)=vv3
                  mm(i  )=mm1
                  ii(i  )=indglob+1 !this is how dice indexes stars (1 is for gas)
                  tmppart%family = 2 !2 is star
                  tmppart%tag    = 0
                  pp(i  )=part2int(tmppart)
                else !meaning gas particle
                  read(10,*,end=100),mm1,xx1,xx2,xx3,vv1,vv2,vv3,uu1,zz1,hh1,yy1,ee1
                  jpart=jpart+1
                  indglob=indglob+1
                  xx(i,1)=xx1+boxlen/2.0
                  xx(i,2)=xx2+boxlen/2.0
                  ...
                  vv(i,3)=vv3
                  mm(i  )=mm1
                  ii(i  )=1 !this is how dice flags gas particles
                  tmppart%family = 100 !random number for gas particles
                  tmppart%tag    = 0
                  pp(i  )=part2int(tmppart)                        
                  uu(i)=uu1 !thermal energy density - erg/cm3, but in ramses units
                  zz(i)=zz1 !metallicity
                  hh(i)=hh1 !HI - mass fraction
                  yy(i)=yy1 !HeI - mass fraction
                  ee(i)=ee1 !HeII - mass fraction
                endif
             end do
100          continue
    ...
    nstar_tot = npart_cpu(ncpu)
    if(debug)write(*,*)'npart=',npart,'/',npart_cpu(ncpu)

    print*, myid, npart_cpu(myid), npart_all(myid)

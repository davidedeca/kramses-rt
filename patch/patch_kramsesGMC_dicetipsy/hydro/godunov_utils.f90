...
subroutine hydro_refine(ug,um,ud,ok,nn)
...
#endif
#if defined(krome_on)
   use krome_user, ONLY:krome_nmols,KROME_idx_H,KROME_idx_Hj, KROME_idx_H2
   use amr_commons, ONLY:units_density
#endif
  implicit none
  ! dummy arguments
  integer nn
  ...
  if(rt_err_grad_xHII >= 0.) then !---------------------------------------
     do k=1,nn
#ifdef krome_on
        dg=min(1.d0,max(0.d0,ug(k,krome_id_vec(KROME_idx_Hj))))
        dm=min(1.d0,max(0.d0,um(k,krome_id_vec(KROME_idx_Hj))))
        dd=min(1.d0,max(0.d0,ud(k,krome_id_vec(KROME_idx_Hj))))
#else
  ...
  if(rt_err_grad_xHI  >= 0.) then !---------------------------------------
     do k=1,nn
#ifdef krome_on
        dg=min(1.d0,max(0.d0,ug(k,krome_id_vec(KROME_idx_H))))
        dm=min(1.d0,max(0.d0,um(k,krome_id_vec(KROME_idx_H))))
        dd=min(1.d0,max(0.d0,ud(k,krome_id_vec(KROME_idx_H))))
#else
  ...
  ! Molecular state (only Hydrogen)
  if(rt_err_grad_xH2  >= 0.) then !---------------------------------------
     do k=1,nn
#ifdef krome_on
        dg=min(1.d0,max(0.d0,ug(k,krome_id_vec(KROME_idx_H2))))
        dm=min(1.d0,max(0.d0,um(k,krome_id_vec(KROME_idx_H2))))
        dd=min(1.d0,max(0.d0,ud(k,krome_id_vec(KROME_idx_H2))))
#else
        dg=min(1.d0,max(0.d0,1.d0 - ug(k,iIons)))
        dm=min(1.d0,max(0.d0,1.d0 - um(k,iIons)))
        dd=min(1.d0,max(0.d0,1.d0 - ud(k,iIons)))
#endif
        error=2.0d0*MAX( &
             & ABS((dd-dm)/(dd+dm+rt_floor_xH2)) , &
             & ABS((dm-dg)/(dm+dg+rt_floor_xH2)) )
        ok(k) = ok(k) .or. error > rt_err_grad_xH2
     end do
  end if
#endif
...
!#########################
  if(H2_d >=0.) then
     do k=1,nn
#ifdef krome_on
        dm=um(k,krome_id_vec(KROME_idx_H2))*um(k,1)
#else
        dm=um(k,iIons)
#endif
        ok(k) = ok(k) .or. dm > H2_d
     end do
  end if

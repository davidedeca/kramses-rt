...
!!! DICE
    ...
    do ifile=1,ic_nfile
       ...
       if(myid==1)then
          ...
          if((ic_format.ne.'Gadget1').and.(ic_format.ne.'Gadget2') .and. (ic_format.ne.'Gasoline')) then
             if(myid==1) write(*,*) 'Specify a valid IC file format [ic_format=Gadget1/Gadget2/Gasoline]'
             error=.true.
          endif
          OPEN(unit=1,file=filename,status='old',action='read',form='unformatted',access="stream")
   
      if ((ic_format .eq. 'Gadget1') .or. (ic_format .eq. 'Gadget2')) then 
  ...
  elseif(ic_format .eq. 'Gasoline') !if file is Gasoline (actually my format...)

    !read header
    !.. here it should read the number of particles

    nstar_tot = 0
    npart     = !see what has been read in the header
    ngas      = npart !reading only gas particles here
    nhalo     = 0
    ...
    eob      = .false.  ! I think kind of end of file, I'm not using it here
    kpart    = 0      
    lpart    = 0
    mpart    = 0
    gpart    = 0
    ...
    do while(.not.eob)
       ...
       if(myid==1)then
          jpart=0
          do i=1, nvector
             jpart=jpart+1  !particle counter
             type_index = 1 ! appropriate for gas particle
             
             read( XX , POS = XX) xx_sp(i, 1:3)  !read particle pos from file 
             read( XX , POS = XX) vv_sp(i, 1:3)  !read particle vel from file
             read( XX , POS = XX) mm_sp(i, 1:3)  !read particle mass from file
             read( XX , POS = XX) uu_sp(i, 1:3)  !read particle temp from file
             read( XX , POS = XX) zz_sp(i, 1:3)  !read particle Z from file
             !! add other properties to read from tipsy files (e.g. aux files)

             ii(i) = jpart

             !scale to ramses code units
             xx(i,:) = xx_sp(i,:) *    !! complete based on original units
             vv(i,:) = vv_sp(i,:) *
             mm(i)   = mm_sp(i)   *
             uu(i)   = uu_sp(i)   *    !! temperature, in K/mu (maybe)
             zz(i)   = zz_sp(i)   *    !! maybe as fraction, not Zsun? check..  
             
 
                !! maybe shift position by boxlen?

             mgas_tot = mgas_tot + mm(i) 

          enddo !end do i=1,nvector
       endif !end if (myid = 1)
       ...
       call MPI_BCAST(uu   , nvector  , MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,info)
       call MPI_BCAST(jpart, 1        , MPI_INTEGER         ,0,MPI_COMM_WORLD,info)
       call MPI_BCAST(npart, 6        , MPI_INTEGER         ,0,MPI_COMM_WORLD,info)
       call cmp_cpumap(xx,cc,jpart)
#endif
       ...
       do i=1,jpart
          ...
          if(cc(i)==myid)then
              ...
              !! here there is a skip if particle out of the box.. i think it's
              !! better without
  ...
  endif !end if file is Gadget1, Gadget2 or Gasoline

    enddo ! end do ifile=1,ic_nfile       

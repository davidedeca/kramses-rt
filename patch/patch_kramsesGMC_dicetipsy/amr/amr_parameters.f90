module amr_parameters
  ...
  real(dp)::CR_ionisation_rate=6.d-17
  real(dp)::H2_dissociation_rate=2.07e9  ! Richings2014 approximation, for a flat FUV spectrum
  ...
  real(dp)::t_starformation=0.d0
  integer::max_star_number=0
  real(dp)::eta_star=0.1
  real(dp)::m_star_average=2.
  real(dp)::m_remnant=1.4
  real(dp)::energy_sn=1.d51


#if defined(krome_on)
  logical ::krome_chem=.true. ! Non-equilibrium chemistry with KROME
#else
  logical ::krome_chem=.false. ! Non-equilibrium chemistry with KROME
#endif
  ! sarebbe da mettere in hydro_parameters in realta, a voler seguire le convenzioni
  integer ::krome_off = 0
  real(kind=8),parameter::krome_met_low = 1.d-7 ! metallicity cut for krome

  ! Output times
  real(dp),dimension(1:MAXOUT)::aout=1.1       ! Output expansion factorsi
  ...
  real(dp),dimension(1:MAXLEVEL)::star_refine = -1.0
  ...
  character(LEN=20)::filetype='none'
  real(dp), dimension(1:MAXLEVEL)::file_x_offset=0.0
  real(dp), dimension(1:MAXLEVEL)::file_y_offset=0.0
  real(dp), dimension(1:MAXLEVEL)::file_z_offset=0.0
  integer, dimension(1:MAXLEVEL)::file_ncell

  ! Cloud carving
  integer  ::carving_restart = 999999
  real(dp) ::carving_radius  = 0d0 

...
subroutine coolfine1(ind_grid,ngrid,ilevel)
...
#if defined(dissipate_turbulence) && NENER>0
  use physical_constants, ONLY: parsec,kms_cgs
#endif

#if defined(krome_on)
  use krome_main !mandatory
  use krome_user !array sizes and utils
  use krome_photo, ONLY: get_photoIntensity
  use amr_parameters, ONLY: krome_off,krome_met_low
#endif                     

#if defined(krome_on) && defined(uniform_radiation)
  use kramses_module,     ONLY: constant_Habing_field
  use physical_constants, ONLY: habing2draine
#endif
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
  ...
  real(kind=8)::dtcool,damp_factor,t_blast
#if defined(dissipate_turbulence) && NENER>0
  real(kind=8)                           :: tau_diss_scale
  real(kind=8),parameter                 :: t_dissipation   = 9.785*1d6*(365.*24.*3600.)  ! ~10 Myr
  real(kind=8),dimension(1:nvector),save :: diss_factor
#endif
#if !defined(krome_on)
  real(kind=8):: nISM,nCOM,cooling_switch
  real(kind=8),dimension(1:nvector),save::T2min,boost
  real(dp)::polytropic_constant=1.
  real(kind=8),dimension(1:nvector),save::delta_T2
#endif
  integer,dimension(1:nvector),save::ind_cell,ind_leaf
  real(kind=8),dimension(1:nvector),save::nH,T2,ekk,err,emag
  real(kind=8),dimension(1:nvector),save::Zsolar
  real(dp),dimension(1:3)::skip_loc
  real(kind=8)::dx,dx_loc,scale,vol_loc
  real(dp),parameter::avg_sfr             = 1.
#ifdef RT
  ...
  real(kind=8)::n_norm_H
  real(dp),dimension(1:krome_nmols)::krome_scale,inv_krome_scale
  real(kind=8)::temperatura_cmb
  integer,dimension(1:krome_nmols) ::krome_id_vec
  integer:: i_krome
  real*8::unoneq(1:krome_nmols), tgas
  real*8::mu_noneq,mu_noneq_old,T2old,t2gas

#if !defined(RT)
  real(dp),dimension(krome_nmols, 1:nvector),save:: xion   ! actually usage can be avoided
  real(kind=8),dimension(1:nvector),save::T2_new           ! same here 
#endif
  !
#if defined(uniform_radiation)
  real(kind=8)::average_draine_intensity
#endif
  !
#endif
  ...
  ! SFR dependent settings: krome_set_user_crate(avg_sfr * 3d-17)
  call krome_set_user_crate(CR_ionisation_rate)
  call krome_set_user_myH2_dissociation(H2_dissociation_rate)
#endif

#if !defined(RT) && defined(uniform_radiation) && defined(krome_on)
  ! set a spatially constant field
  ! please use it for test only
  ! currently the shape is set in the krome init wrapper (habing field)
  call krome_photoBin_restore()
  average_draine_intensity = habing2draine*max(0.0,constant_Habing_field)
  ! for cosmo, a tweak is needed
  call krome_photoBin_scale(average_draine_intensity)
#endif
...
#if defined(dissipate_turbulence) && NENER>0
  ! definitionin  Mac Low 1999
  ! used in Pallottini+17
  ! dtau_dissipation = dt / (10Myr * (L/100 pc) * (10 kms/velocity))
  tau_diss_scale = dtcool/(t_dissipation*(dx_loc*scale_l/(1d2*parsec))*(10*kms_cgs/scale_v))
#endif
  ...
  do ind=1,twotondim
     ...
     ! Compute non-thermal energy contributtion
     do i=1,nleaf
        err(i)=0.0d0
     end do
#if NENER>0
...
#if defined(dissipate_turbulence)
     ! get the multiplication factor for the backward euler 1st order
     ! it s fine but eventually i should put a second order, just to be safe
     do i=1,nleaf
        diss_factor(i) = 1.0/(1.0 + dtau_dissipation* max(sqrt(err(i)/nH(i)),0.0))
     enddo
#endif

#endif

     ! Compute MHD energy contributtion
     do i=1,nleaf
        emag(i)=0.0d0
     end do
#ifdef SOLVERmhd
...
!if(myid==1) print*, 'before', uold(ind_leaf(1), krome_id_vec(:))
     ...
     do i=1,nleaf  
       ...
       enddo
       ! KROME: from code units of RAMSES to KROME 1/cm3
       unoneq(1:krome_nmols) = unoneq(1:krome_nmols)*krome_scale(1:krome_nmols)
       xion(1:krome_nmols,i) = unoneq(1:krome_nmols)
       ! convert to K
       mu_noneq_old          = krome_get_mu(unoneq(1:krome_nmols))
       T2(i)                 = T2(i) * mu_noneq_old
     end do
#endif

#if defined(RT) && !defined(krome_on)
     if(neq_chem) then
        do ii=0,nIons-1
           do i=1,nleaf
     ...
     ! krome can be called from within the rt_solve_cooling
     if(neq_chem) then
...
#if defined(krome_on) && !defined(grackle) && !defined(RT) 
      ! rt cooling is off, calling krome here
      do i=1,nleaf   
      
        unoneq(1:krome_nmols) = xion(1:krome_nmols,i)
        tgas                  = T2(i)

        call krome_set_Z(Zsolar(i))
        call krome_set_dust_to_gas(Zsolar(i))
...
#ifdef RT
     !if(.not. static .and. t > static_time) then
     if(.not. static) then
     ...
     do irad=0,nener-1
        do i=1,nleaf
           uold(ind_leaf(i),inener+irad)  = diss_factor(i)*uold(ind_leaf(i),inener+irad)
        end do
     end do
     do i=1,nleaf
       err(i) = diss_factor(i) * err(i)
     enddo
     ! physical relevant question:
     ! should we use the dissipated unresolved turbulence as an heating rate for this timestep?
#endif

#if defined(krome_on) 
     ! convert back gas temperature to thermal energy
     do i=1,nleaf
        T2(i)    = max(T2_new(i), temperatura_cmb)
        mu_noneq = krome_get_mu(xion(:,i))
        T2(i)    = T2(i)/mu_noneq
        T2(i)    = T2(i)*nH(i)*energy_norm
     end do
     ! sum thermal, kinetic and non thermal energy
     do i=1,nleaf
        uold(ind_leaf(i),ndim+2) = T2(i)+ekk(i)+err(i)+emag(i)
     end do
#else
     ...
     do i_krome =1,krome_nmols
        do i=1,nleaf
           uold(ind_leaf(i),krome_id_vec(i_krome)) = xion(i_krome,i)*inv_krome_scale(i_krome)
        end do
     enddo
#endif

!if(myid==1) print*, 'after', uold(ind_leaf(1), krome_id_vec(:))

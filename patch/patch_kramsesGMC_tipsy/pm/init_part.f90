subroutine init_part
  ...
  real(dp)::dx,xx1,xx2,xx3,vv1,vv2,vv3,mm1,uu1,ee1,hh1,zz1,yy1
  ...
  integer(i8b),dimension(1:ncpu)::npart_gas_cpu,npart_gas_all
  integer(i8b),dimension(1:ncpu)::npart_star_cpu,npart_star_all
  ...
  real(kind=8),dimension(1:nvector)::mm,ee,yy,hh,zz,uu
  type(part_t)::tmppart
...
#endif
  logical::error,keep_part,eof,read_pos=.false.,ok, ok_file
  ...
  allocate(uthp    (npartmax))
  allocate(hp    (npartmax))
  allocate(yp    (npartmax))
  allocate(ep    (npartmax))
  ...
  if(nrestart>0)then
     ...
     ok_file = .false.
     INQUIRE(file=fileloc,exist=ok_file)
     if(ok_file) then
     ...
     read(ilun)nsink
     if(myid==1) print*, '>>>> STARS LOADED', nstar_tot
     ...
     star_line = nstar_tot + 1

  else  ! if part outputs not found
...
!#################################
  ...
  subroutine load_ascii
    ...
    if(TRIM(initfile(levelmin)).NE.' ')then
       filename=TRIM(initfile(levelmin))//TRIM('/ic_sph.dat')
       if(myid==1)then
       ...
       do while (.not.eof)
          ...
          if(myid==1)then
             jpart=0
             do i=1,nvector
                read(10,*,end=101)mm1,xx1,xx2,xx3,vv1,vv2,vv3,uu1,zz1,hh1,yy1,ee1
                jpart=jpart+1
                !indglob=indglob+1
                ...
                mm(i  )=mm1
                ii(i  )=1 !indglob
                tmppart%family = 100!FAM_PART
                tmppart%tag    = 0
                uu(i)=uu1 !thermal energy density - erg/cm3, but in ramses un
                zz(i)=zz1 !metallicity
                hh(i)=hh1 !HI - mass fraction
                yy(i)=yy1 !HeI - mass fraction
                ee(i) = ee1                
...
101          continue
          ...
          call MPI_BCAST(hh,nvector ,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,info)
          call MPI_BCAST(yy,nvector ,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,info)
          call MPI_BCAST(ee,nvector ,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,info)
    ...
    if(TRIM(initfile(levelmin)).NE.' ')then
       filename=TRIM(initfile(levelmin))//TRIM('/ic_part.dat')
       if(myid==1)then
          open(10,file=filename,form='formatted')
          indglob=0
       end if
       ...
       do while (.not.eof)
          ...
          if(myid==1)then
             do i=1,nvector
                read(10,*,end=100)mm1,xx1,xx2,xx3,vv1,vv2,vv3
                jpart=jpart+1
                indglob=indglob+1
                xx(i,1)=xx1+boxlen/2.0
                xx(i,2)=xx2+boxlen/2.0
...
100          continue
    ...
    npart_gas_cpu=0; npart_gas_all=0
    npart_gas_cpu(myid)=ipart_gas

    npart_star_cpu=0; npart_star_all=0
    npart_star_cpu(myid)=ipart_star

#ifndef WITHOUTMPI
#ifndef LONGINT
    call MPI_ALLREDUCE(npart_cpu,npart_all,ncpu,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
    call MPI_ALLREDUCE(npart_gas_cpu,npart_gas_all,ncpu,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
    call MPI_ALLREDUCE(npart_star_cpu,npart_star_all,ncpu,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
#else
    call MPI_ALLREDUCE(npart_cpu,npart_all,ncpu,MPI_INTEGER8,MPI_SUM,MPI_COMM_WORLD,info)
    call MPI_ALLREDUCE(npart_gas_cpu,npart_gas_all,ncpu,MPI_INTEGER8,MPI_SUM,MPI_COMM_WORLD,info)
    call MPI_ALLREDUCE(npart_star_cpu,npart_star_all,ncpu,MPI_INTEGER8,MPI_SUM,MPI_COMM_WORLD,info)
#endif
    npart_cpu(1)=npart_all(1)
    npart_gas_cpu(1)=npart_gas_all(1)
    npart_star_cpu(1)=npart_star_all(1)
#endif
    do icpu=2,ncpu
       npart_cpu(icpu)=npart_cpu(icpu-1)+npart_all(icpu)
       npart_gas_cpu(icpu)=npart_gas_cpu(icpu-1)+npart_gas_all(icpu)
       npart_star_cpu(icpu)=npart_star_cpu(icpu-1)+npart_star_all(icpu)
    end do

    nstar_tot = npart_star_cpu(ncpu)

    if(debug)write(*,*)'npart=',npart,'/',npart_cpu(ncpu)
 
    if(myid==1)then
       print*, 'TOTAL PARTICLES = ', SUM(npart_all)
       print*, 'TOTAL GAS       = ', SUM(npart_gas_all)
       print*, 'TOTAL STARS     = ', SUM(npart_star_all)
    endif

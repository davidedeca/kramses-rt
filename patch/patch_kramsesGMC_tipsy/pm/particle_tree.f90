...
subroutine virtual_tree_fine(ilevel)
  ...
  !-----------------------------------------------------------------------
#ifndef WITHOUTMPI
  integer::ip,ipcom,npart1,next_part,ncache,ncache_tot, j
  ...
  if(numbtot(1,ilevel)==0)return
  !if(verbose)write(*,111)ilevel
  !if(verbose) print*, 'Entering virtual_tree_fine for level', ilevel, numbtot(1,ilevel)
  ...
  if(dice_init) particle_data_width=particle_data_width+4 !(temp,hi,hei,heii)

  particle_data_width = 13
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  !if(dice_init) then
     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=uthp(ind_part(i))
     end do
     current_property = current_property+1
     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=hp(ind_part(i))
     end do
     current_property = current_property+1
     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=yp(ind_part(i))
     end do
     current_property = current_property+1
     do i=1,np
        reception(icpu,ilevel)%up(ind_com(i),current_property)=ep(ind_part(i))
     end do
     current_property = current_property+1
  !endif
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  !if(dice_init) then
     do i=1,np
       uthp(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property)
     end do
     current_property = current_property+1
     do i=1,np
       hp(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property)
     end do
     current_property = current_property+1
     do i=1,np
       yp(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property)
     end do
     current_property = current_property+1
     do i=1,np
       ep(ind_part(i))=emission(icpu,ilevel)%up(ind_com(i),current_property)
     end do
     current_property = current_property+1
  !endif

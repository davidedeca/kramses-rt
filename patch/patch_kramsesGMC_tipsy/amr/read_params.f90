module dice_commons
  ...
  ! misc
  logical::krome_init_equilibrium=.false.
  integer::n_krome_init    = 100 !default, krome eq. in 100 Myr
  real(dp)::dt_krome_init  = 31556952000000. !seconds in a Myr
...
subroutine read_params
  ...
  namelist/run_params/clumpfind,cosmo,pic,sink,lightcone,poisson,hydro,rt,verbose,debug &
       ...
       & ,static_dm,static_gas,static_stars,static_time,time_shake,convert_birth_times,use_proper_time,remap_pscalar
  namelist/output_params/noutput,foutput,fbackup,aout,tout &
       & ,tend,delta_tout,aend,delta_aout,gadget_output,walltime_hrs,minutes_dump,tfile
  namelist/amr_params/levelmin,levelmax,ngridmax,ngridtot &
       & ,npartmax,nparttot,nexpand,boxlen,nlevel_collapse,mass_sph
...
& ,krome_init_equilibrium, dt_krome_init, n_krome_init &
...
& ,ic_mask_ivar,ic_mask_min,ic_mask_max,ic_mask_ptype, n_star_part  
! MPI initialization
#ifndef WITHOUTMPI
  call MPI_INIT(ierr)
...
106 continue
  ...
  if(tend>0)then
     ...
     if(myid==1) print*, 'reading output times from file'
     open(11, file=trim(tfile), form='formatted')
     i = 1
     do
       read(11, *, iostat=ios) tout(i)
       if (ios /= 0 .or. i == MAXOUT) exit
       i = i + 1
     enddo
     noutput = i-1
     close(11)
  ...
  do i=nlevelmax,levelmin,-1
     ...
     star_refine(i)=star_refine(i-levelmin+1)
  end do
  do i=1,levelmin-1
     nexpand   (i)= 1
     nsubcycle (i)= 1

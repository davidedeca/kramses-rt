...
#endif
  integer::ilevel,idim,ivar,j,ipart

  integer, allocatable, dimension(:) :: ind_grid1
  integer :: ibound, igrid, istart, ncache, iskip, ind, i
...
!!DICE: SHAKE A BIT
  if (time_shake>0) then
   if(t<time_shake) then
      !cooling=.false.
      static=.false.
      if(myid==1) print*, '.. shaking is on'
   else
      !cooling=.true.
      static=.true.
      time_shake=-1d0

...
!-------------------------------------------------------------------------
  namelist/rt_params/rt_star, rt_esc_frac, rt_flux_scheme, rt_smooth     &
       ...
       & ,rt_uv_background, rt_n_background
...
!-------------------------------------------------------------------------
  ...
  use SED_module
  implicit none
  real(dp), dimension(10)::groupL0_default, groupL1_default, group_egy_default
  integer::i
!-------------------------------------------------------------------------
  ...
  groupL0_default   =  (/0.7542d0, 2.65d0, 6.0d0 ,  11.2d0,  13.6d0  , 14.159d0, 15.4d0 ,  24.59d0, 30.0d0 ,  54.42d0/)
  groupL1_default   =  (/2.65d0  , 6.0d0 , 11.2d0,  13.6d0,  14.159d0, 15.4d0  , 24.59d0,  30.0d0 , 54.42d0,  1000.0d0/)
  group_egy_default =  (/1.96300977623 , 4.48983275063 , 8.37722424786 , 12.3017463205 ,13.8734719772 , 14.7482029986 , 18.1673223329 , 26.5186427192 , 32.7650226189 , 56.94390135/)

  groupL0(1:min(nGroups, 10)) = groupL0_default(1:min(nGroups, 10))
  groupL1(1:min(nGroups, 10)) = groupL1_default(1:min(nGroups, 10))
  group_egy(1:min(nGroups, 10)) = group_egy_default(1:min(nGroups, 10))
       ...
       ! Exponent of choosen norm
        ...
        do i=1,nn
           ...
           if(r<1.0)then
              ...
              uu(i,group_ind+1) = rt_u_source(k) * rt_c * rt_n_source(k) / scale_Np
#if NDIM>1

...
subroutine userflag_fine(ilevel)
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        if(star_refine(ilevel)>-1.0)then
          do idim=1,ndim
             do i=1,ngrid
                xx(i,idim)=xg(ind_grid(i),idim)+xc(ind,idim)
             end do
          ...
          call stellar_refine(xx,ok,ngrid,ilevel)
        end if
...
!#####################################################################
!#####################################################################
subroutine stellar_refine(xx,ok,ncell,ilevel)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
  implicit none
  integer::ncell,ilevel,nx_loc,ipart
  real(dp),dimension(1:nvector,1:ndim)::xx
  logical ,dimension(1:nvector)::ok
  real(dp)::er,xn,yn,zn,r,scale
  integer ::i
     ...
     zn=xx(i,3)
        ...
        do ipart=1,npartmax
           if ( is_star(typep(ipart)) ) then
             !r  = ((xn-xp(ipart, 1))**2+(yn-xp(ipart, 2))**2+(zn-xp(ipart, 3))**2)**(1.0/2.0)
             nx_loc=(icoarse_max-icoarse_min+1)
             scale=boxlen/dble(nx_loc)
             er = 3.d0/2.d0 * 0.5D0**(ilevel+1) * scale
             if(abs(xn-xp(ipart, 1))<er  .and. abs(yn-xp(ipart, 2))<er .and. abs(zn-xp(ipart, 3))<er)then
               ok(i)=.true.
             endif
           endif
        end do

...
subroutine count_leaf_cells()
  ...
  integer ::ncache,nnew,ivar,ngrid,ilevel,icpu,info
  integer ::igrid,ix,iy,iz,ind,i,n,iskip,nx_loc,idim
  integer ::ntot,ntot_all,nstar_corrected,ncell,ntot_cpu
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  logical ::ok_free
  real(dp)::d,x,y,z,dH2
  real(dp)::T2,nH,T_poly,cs2,cs2_poly,trel,t_dyn,t_ff,tdec,uvar
  real(dp)::cell_mass, dm_star
  real(kind=8)::mlost_all,mtot_all
  real(dp),dimension(1:3)::skip_loc
  real(dp)::dx,dx_loc,scale,vol_loc,dx_min,vol_min,d1,d2,d3,d4,d5,d6,dt
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
          if(ok(i))then
             d=uold(ind_cell(i),1)
             dh2=uold(ind_cell(i), krome_off+KROME_idx_H2)
             if(dh2>n_star*mH/scale_d) then
               leaf_cell_cut_cpu = leaf_cell_cut_cpu + d * dx_loc**3 * SQRT(d)
             endif
             if(dh2>10*mH/scale_d) then
               leaf_cell_tot_cpu = leaf_cell_tot_cpu + d * dx_loc**3 * SQRT(d)
             endif
          end if
        end do
     end do
...
#ifndef WITHOUTMPI
    call MPI_ALLREDUCE(leaf_cell_cut_cpu, leaf_cell_cut, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, info)
    call MPI_ALLREDUCE(leaf_cell_tot_cpu, leaf_cell_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, info)
#endif

end subroutine count_leaf_cells


subroutine star_formation(ilevel, dt)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
  ...
  !----------------------------------------------------------------------
  ! local constants
  real(dp)::t0,d0,d00,dh2,mgas,mcell
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  ...
  real(dp)::dx,dx_loc,scale,vol_loc,dx_min,vol_min,d1,d2,d3,d4,d5,d6,dt
  real(dp)::mdebris
  real(dp)::prob,SFRd,t_freefall
  real(dp),dimension(1:nvector)::sfr_ff
  integer ,dimension(1:ncpu,1:IRandNumSize)::allseed
  real(dp), dimension(1:nvector)::mstar_list
  integer ,dimension(1:nvector),save::ind_grid,ind_cell,ind_cell2,nstar
  integer ,dimension(1:nvector),save::ind_grid_new,ind_cell_new,ind_part
  integer ,dimension(1:nvector),save::ind_debris
  integer ,dimension(1:nvector,0:twondim)::ind_nbor
  logical ,dimension(1:nvector),save::ok,ok_new=.true.
  integer ,dimension(1:ncpu)::ntot_star_cpu,ntot_star_all,nstar_cpu
  character(LEN=80)::filename,filedir,fileloc,filedirini,filestars
  character(LEN=5)::nchar,ncharcpu
  logical::file_exist, ok_file
  integer::ios,iline,nlines,index_p
  integer :: values(1:8), k_random
  integer, dimension(:), allocatable :: seed
  real(8) :: r_random
  real(dp)::eta_star1, d_old
#ifdef SOLVERmhd
  ...
  if(ndim.ne.3)return
  if(static)return
  if (t<t_starformation)return
  if (star_line > max_star_number)then
    if(mstar_tot < 1.e-10) then
      return
    else
      GOTO 666  !remove mass from previous fine timesteps
    endif
  endif

  if (myid==1)print*,'--- STAR FORMATION --- '
  ...
  d0   = nISM * mH/scale_d
  d00  = n_star * mH /scale_d

  !call date_and_time(values=values)
  !call random_seed(size=k_random)
  !allocate(seed(1:k_random))
  !seed(:) = values(8)
  ...
  if(.not.allocated(star_list)) then
   filestars=trim(SED_DIR)//'/star_list.dat'
   inquire(file=trim(filestars), exist=ok_file)
   if(ok_file .eqv. .false.)then
    write(*,*) 'Cannot read star_list.dat file'
    call clean_stop
   else

    open(11, file=trim(filestars), form='formatted')
    nlines=0
    do
      read(11,*, END=128)
      nlines = nlines + 1
    ...
    do
      ...
      read(11, *, iostat=ios) star_list(iline,:)
      if (ios /= 0) exit
      iline = iline + 1
    end do
    close(11)
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        if(sf_virial)then
           ...
           ! here all ok(i) are true. Applying density threshold + uniform probability
           do i=1,ngrid
              nstar(i)=0
              if(.not. ok(i)) cycle
              d=uold(ind_cell(i), 1)

              dh2=uold(ind_cell(i), krome_off+KROME_idx_H2)*d 
              if(dh2<=d0)then
                ok(i)=.false.
              else
                t_freefall = 1.66 / SQRT(d * scale_d / 1.67d-24 / 1.d3) ! Myr
                cell_mass = d * scale_d * (dx_loc*scale_l)**3 / 2.d33 ! Msun
                eta_star1 = eta_star * leaf_cell_tot / leaf_cell_cut
                SFRd    = eta_star1 * cell_mass / t_freefall ! Msun/Myr
                dm_star = SFRd * dtnew(ilevel) ! Msun
                prob = dm_star / m_star_average
                call ranf(localseed,r_random)
                if(prob < r_random)then
                  ok(i)=.false.
                else
                  nstar(i) = 1
                  ntot = ntot + 1
        ...
        ! do i=1,ngrid          !!!! tstar non viene più usato / mstar_tot può essere calcolato dopo
        !   mstar_list(i)=0
        !   if (ok(i))then
        !     mstar_list(i) = star_list(iline,1)
        !     tstar= .5427*sqrt(1.0/(factG*max(d,smallr)))
        !     mstar_tot=mstar_tot+mstar_list(i)
        !     iline = iline + 1
        !   end if
        ! end do
...
#ifndef WITHOUTMPI
  call MPI_ALLGATHER(ntot, 1, MPI_INTEGER, nstar_cpu, 1, MPI_INTEGER, MPI_COMM_WORLD, info)
#endif
  iline = star_line + SUM(nstar_cpu(1:(myid-1)))
  star_line = star_line + SUM(nstar_cpu(:))
...
!  mlost_all=mstar_lost
  ...
  if(myid==1) then
    write(*,'("Star formed in the current timestep",I6)') ntot_all
    write(*,'("Total number of stars",I6)') nstar_tot
  endif
!  if(myid==1)then
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           if(iline > max_star_number)then
             iline = iline - max_star_number
           endif
           ...
           mp(ind_part(i)) = star_list(iline,1)      ! Mass
           levelp(ind_part(i)) = ilevel   ! Level
           idp(ind_part(i)) = index_p !index_star  ! Star identity
           ...
           print *,'Star formed: ',idp(ind_part(i)),mp(ind_part(i))
           iline = iline + 1

           mstar_cpu = mstar_cpu + mp(ind_part(i))
           print*, mp(ind_part(i)), mstar_cpu
...
#ifndef WITHOUTMPI
        call MPI_ALLREDUCE(mstar_cpu, mstar_tot, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, info)
#endif
...
!##### MASS REMOVAL AFTER STAR FORMATION

666 CONTINUE

  if (ilevel==levelmin)then

  !write (filename, "(A4,I3)") "out_", myid
  !print*, 'saving to '//filename
  !OPEN(112, file=filename)

  leaf_cell_vol_cpu=0
  leaf_cell_vol=0

  do jlevel=levelmin,nlevelmax
  ...
  ncache=active(jlevel)%ngrid
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     do i=1,ngrid
        ind_grid(i)=active(jlevel)%igrid(igrid+i-1)
     end do
     do ind=1,twotondim
        iskip=ncoarse+(ind-1)*ngridmax
        do i=1,ngrid
        ...
        do i=1,ngrid
             if(ok(i))then
              dh2=uold(ind_cell(i), krome_off+KROME_idx_H2)
              if(dh2>10.*mH/scale_d) then
                leaf_cell_vol_cpu = leaf_cell_vol_cpu + dx_loc**3 * uold(ind_cell(i), 1)
              endif
             endif
        end do
     end do
...
#ifndef WITHOUTMPI
      call MPI_ALLREDUCE(leaf_cell_vol_cpu, leaf_cell_vol, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, info)
#endif

  dstar = mstar_tot / leaf_cell_vol

  if(myid==1) print*, 'removing mass after star formation.. Mstar_tot=', mstar_tot, ' n_removed=', dstar, 'number_cell_removal=', leaf_cell_vol
  ...
  do igrid=1,ncache,nvector
    ...
    do ind=1,twotondim
       ...
       do i=1,ngrid
        if(ok(i))then
         dh2=uold(ind_cell(i), krome_off+KROME_idx_H2)
         d_old = uold(ind_cell(i), 1)
         if(dh2>10.*mH/scale_d) then
           do ivar=ndim+3+nener, nvar
             uold(ind_cell(i), ivar) = uold(ind_cell(i),ivar) * ( 1 - dstar)
           enddo  
           uold(ind_cell(i), 5) = uold(ind_cell(i), 5) - 0.5*(uold(ind_cell(i), 2)**2 + uold(ind_cell(i), 3)**2 + uold(ind_cell(i), 4)**2)/uold(ind_cell(i), 1)
           uold(ind_cell(i), 1) = uold(ind_cell(i), 1) * (1 - dstar)
           uold(ind_cell(i), 5) = uold(ind_cell(i), 5) + 0.5*(uold(ind_cell(i), 2)**2 + uold(ind_cell(i), 3)**2 + uold(ind_cell(i), 4)**2)/uold(ind_cell(i), 1)  
           !write(112, *) dx_loc, d_old, uold(ind_cell(i), 1)
         endif
        endif
       end do

...
subroutine init_flow_fine(ilevel)
  ...
  !!! IF NOT COSMO
    ...
    open(10,file=TRIM(filename)//TRIM('_xyzh.dat'),form='unformatted')
    read(10) nsph
    allocate(tipsyarray_xyzh(5, nsph))
    read(10) tipsyarray_xyzh
    close(10)

    tipsyarray_xyzh(1,:) = tipsyarray_xyzh(1,:) * 1000. + boxlen/2.
    tipsyarray_xyzh(2,:) = tipsyarray_xyzh(2,:) * 1000. + boxlen/2.
    tipsyarray_xyzh(3,:) = tipsyarray_xyzh(3,:) * 1000. + boxlen/2.
    tipsyarray_xyzh(4,:) = tipsyarray_xyzh(4,:) * 1000.


if(myid==1)print*, 'interpolation of IC, level', ilevel

    !!! LOOP OVER VARIABLES
    do ivar=1,nvar

      if(myid==1) print*, '.. variable', ivar, '/', nvar

      !load different files for each variable
      if(ivar==1)then
          filevar=TRIM(filename)//TRIM('_rho.dat')
      elseif(ivar==2)then
          filevar=TRIM(filename)//TRIM('_vx.dat')
      elseif(ivar==3)then
          filevar=TRIM(filename)//TRIM('_vy.dat')
      elseif(ivar==4)then
          filevar=TRIM(filename)//TRIM('_vz.dat')
      elseif(ivar==5)then
          filevar=TRIM(filename)//TRIM('_temp.dat')
      elseif(ivar==6)then
          filevar=TRIM(filename)//TRIM('_metals.dat')
      elseif(ivar==7)then
          filevar=TRIM(filename)//TRIM('_HI.dat')   !x1 (HI)
      elseif(ivar==8)then
          filevar=''                          !x2 (e)
      elseif(ivar==9)then
          filevar=''                          !x3 (HII)
      elseif(ivar==10)then
          filevar=TRIM(filename)//TRIM('_HeI.dat')  !x4 (HeI)
      elseif(ivar==11)then  
          filevar=TRIM(filename)//TRIM('_HeII.dat') !x5 (HeII)
      elseif(ivar==12)then 
          filevar=''                          !x6 (HeIII)
      elseif(ivar==13)then
          filevar=''                          !x7 (H+)
      ...
      if (trim(filevar).ne. '') then
        open(10,file=filevar,form='unformatted')
        read(10) nsph
        allocate(tipsyarray_var(nsph))
        read(10) tipsyarray_var
        close(10)
     endif
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           !call condinit(xx,uu,dx_loc,ngrid,ilevel, ivar, 
           !tipsyarray_xyzh, tipsyarray_var)


        do i=1,ngrid

          if(.not. uold_flag(ind_cell(i))) then

            if (trim(filevar) .ne. '') then
            call interpolate(tipsyarray_xyzh(3,:), &
                  tipsyarray_xyzh(2,:), &
                  tipsyarray_xyzh(1,:), &
                  tipsyarray_xyzh(4,:), &
                  tipsyarray_xyzh(5,:), &
            ...
            uold_flag(ind_cell(i)) = .true.

            
                 !ugly conversion from the interpolated variable (fom sph file)
                 !to the variable ramses wants as ic 


                 if(ivar==2)then       !velocity
                     uu(i,2) = uu(i,2) / scale_v
                 elseif(ivar==3)then   !velocity
                     uu(i,3) = uu(i,3) / scale_v
                 elseif(ivar==2)then   !velocity
                     uu(i,4) = uu(i,4) / scale_v
                 elseif(ivar==5)then  !pressure
                     uu(i,5) = uu(i,1) * kB / krome_init_var(KROME_idx_H) & 
                               * uu(i,5) / scale_d / scale_v**2
                 elseif(ivar==6)then !metal
                     uu(i,6) = uu(i,6) / uu(i,1)
                 elseif(ivar==7)then
                     uu(i,7) = uu(i,7) / uu(i,1)
                 elseif(ivar==8)then
                     if (uu(i,6)<0.1)then
                         Y_He = (0.236 + 2.1*uu(i,6))/4.0
                     else
                         Y_He = (-0.446*(uu(i,6)-0.1)/0.9 + 0.446)/4.0
                     endif     
                     Y_H = 1.0 - Y_He*4.0 - uu(i,6)
                     Y_e = Y_H+ Y_He*2
                     uu(i,8) = Y_e &
                         * krome_init_var(KROME_idx_E)/krome_init_var(KROME_idx_H)
                 elseif(ivar==9)then
                     if (uu(i,6)<0.1)then
                         Y_He = (0.236 + 2.1*uu(i,6))/4.0
                     else
                     ...
                     uu(i,9) = Y_H - uu(i,7)
                 elseif(ivar==10)then
                     uu(i,10) = uu(i,10) / uu(i,1) &
                         * krome_init_var(KROME_idx_HE)/krome_init_var(KROME_idx_H)
                 elseif(ivar==11)then
                     uu(i,11) = uu(i,11) / uu(i,1) &
                         * krome_init_var(KROME_idx_HEj)/krome_init_var(KROME_idx_H)
                 elseif(ivar==12)then
                     if (uu(i,6)<0.1)then
                         Y_He = (0.236 + 2.1*uu(i,6))/4.0
                     else
                     ...
                     uu(i,12) = Y_He*4-uu(i,10)-uu(i,11)
                 elseif(ivar==13)then
                     uu(i,13) = smallr
                 elseif(ivar==14)then
                     uu(i,14) = smallr              
                 ...
                 if(ivar==1)then      !density
                     uu(i,1) = uu(i,1) / scale_d
                 endif
 

        !if ((ivar==1) .and. (xx(i,1)==290625.) .and. (xx(i,2)==290625.) .and. (xx(i,3)==290625.)) then
        !    print*, 'post-unit conv', xx(i,:), uu(i, ivar)
   ...
   if(trim(filevar) .ne. '') deallocate(tipsyarray_var)

   end do ! End loop over variables

   deallocate(tipsyarray_xyzh)
...
!################################################################
!################################################################
subroutine region_condinit(x,q,dx,nn,ilevel, ivar, filevar, tipsyarray_xyzh, tipsyarray_var)
  use amr_commons, ONLY: myid
  use amr_parameters
  use hydro_parameters
  use interpolate_amr
  implicit none
  integer ::nn,ilevel,nline,nsph
  real(dp)::dx, datsmooth
  ...
  ok_file = .true.
       ...
       !!IN KRAMSESGMC_INTERP THE FILE IS THE SPH DATA (TIPSY)
       !!THIS VERSION WORKS ONLY IF YOU GIVE SUCH FILE IN THE NAMELIST

    do i=1,nn

       print*, x(i,1),x(i,2),x(i,3)
       print*, SHAPE(tipsyarray_xyzh)
       print*, MAXVAL(tipsyarray_xyzh)
       print*, q(i, ivar)

       if (filevar .ne. '') then
         call interpolate(tipsyarray_xyzh(1,:), &
                  tipsyarray_xyzh(2,:), &
                  tipsyarray_xyzh(3,:), &
                  tipsyarray_xyzh(4,:), &
                  tipsyarray_xyzh(5,:), &

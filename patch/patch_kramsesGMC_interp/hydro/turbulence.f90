...
!  previous value of iseed. Instead we trust the user to specify the
!  same value
...
!   FIELDS, MAGNETOHYDRODYNAMICS: MHD, SHOCK WAVES, TURBULENCE, ISM:
!   Clouds, ISM: Kinematics and Dynamics, ISM: Magnetic Fields,
...
!  2010 Troels Haugboelle: rewritten to support RAMSES, an AMR grid, and
...
subroutine init_force
  ...
  kmax = ifix(real(k2_force,kind=4)+1.)
  ...
  do jx=-kmax,kmax
     do jy=-kmax,kmax
        do jz=-kmax,kmax
           if (float(jx**2+jy**2+jz**2) .ge. k1_force**2-1e-5 .and. &
               float(jx**2+jy**2+jz**2) .le. k2_force**2+1e-5) nmodes = nmodes + 1
        end do
     end do
  end do

  ! calculate average acceleration
  accel  = ampl_turb/t_turn/sqrt(real(nmodes)/8.0_dp)    ! rms=1./8. per comp.
  ...
  do jx=-kmax,kmax
     do jy=-kmax,kmax
        do jz=-kmax,kmax
           if (float(jx**2+jy**2+jz**2) .ge. k1_force**2-1e-5 .and. &
               float(jx**2+jy**2+jz**2) .le. k2_force**2+1e-5) then
              nmodes = nmodes + 1
...
! Routine for updating the driving modes and calculating averages on
! root grid
!===============================================================================
...
!  Loop until t_turb > t
     do while (t_turb .le. t)
        ak_prev = ak_next
! advances a turnover time
        t_turb = t_turb + t_turn
        if (myid==1) print*,'random_force:t=',myid,t,t_turb,nmodes,iseed_force
        ! To obtain a Kolmogorov slope of the driving alone, one should
        ! have amplitudes
        ! a(k) that drop with k^(-11./6.), to have a(k)^2*k^2 =
        ! k^(-5./3.).  This ASSUMES
        ! that the amplitudes are proportional to the driving.  But the
        ! energy drain may
        ! be rather inversely proportional to the turnover time, which
        ! goes as k^(2./3.)
        ...
        do jx=-kmax,kmax
           ...
           do jy=-kmax,kmax
              ...
              do jz=-kmax,kmax
                 ...
                 if (fk .ge. k1_force-1e-5 .and. fk .le. k2_force+1e-5) then
                    lfxx = exp(cmplx(0., 2.*pi*ran1s(iseed_force)))/fk**pk_force
                    lfyy = exp(cmplx(0., 2.*pi*ran1s(iseed_force)))/fk**pk_force
                    lfzz = exp(cmplx(0., 2.*pi*ran1s(iseed_force)))/fk**pk_force
...
!--------------------------------------------------
  ...
  ! Loop over grids
  !!$omp parallel do
  !private(igrid,i,ngrid,ind_grid,ind,iskip,ivar,ind_cell,uu,x,y,z,j,expikr)
  !&
  !!$omp&
  !reduction(+:rhoavp,pxavp,pyavp,pzavp,aranxp,aranyp,aranzp) &
  !!$omp&            default(none)
  !shared(ncache,active,ncoarse,ngridmax,uold,xg,xc,nmodes,kx,ky,kz,fxx,fyy,fzz,ilevel)
  ...
  call MPI_ALLREDUCE(comm_buffin,comm_buffout,7,MPI_DOUBLE_PRECISION,MPI_SUM, MPI_COMM_WORLD,info)
  ...
  if (myid==1 .and. verbose) write(*,'(a,4e12.4)') 'rho_av, xav, yav, zav :', &
     (/ rhoav /(real(numbtot(1,levelmin))*twotondim),aranx,arany,aranz/)
...
subroutine do_random_force(ilevel,dt)
 ...
 dx = 0.5D0**ilevel
  ...
  !private(igrid,ngrid, &
  !!$omp&
  !i,ind_grid,ind,iskip,nleaf,j,ind_cell,rho,ein,x,y,z, &
  !!$omp&            expikr,ax,ay,az)
  !shared(ncache,active,ilevel,ncoarse,&
  !!$omp&
  !ngridmax,son,uold,xg,xc,nmodes,kx,ky,kz,fxx,fyy,fzz, &

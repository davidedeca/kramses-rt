subroutine init_hydro
  ...
  real(dp)::gamma2, x, y, z, radius, scale, dx
  ...
  allocate(uold_flag(1:ncell))
  uold=0.0d0; unew=0.0d0 ; uold_flag=.false.
           ...
           if(ncache>0)then
              ...
              do ind=1,twotondim
                 ...
                 do ivar=1,ndim+1
                    read(ilun)xx
                    if(ivar==1)then
                      if(nrestart==carving_restart) then !if carving
                       do i=1,ncache
                          x=(xg(ind_grid(i),1)+xc(ind,1)-skip_loc(1))*scale
                          y=(xg(ind_grid(i),2)+xc(ind,2)-skip_loc(2))*scale
                          z=(xg(ind_grid(i),3)+xc(ind,3)-skip_loc(3))*scale
                          radius = (x-30.)**2 + (y-30.)**2 + (z-30.)**2
                          if (radius > carving_radius**2) then
                            !uold(ind_grid(i)+iskip,1)=xx(i) * ( 0.99 * (radius/15d0)**2 - 3.96 * (radius/15d0) + 3.97 )
                            uold(ind_grid(i)+iskip,1)=xx(i) / 1d4
                          else
                 ...
                 read(ilun)xx
                 do i=1,ncache
                     if(carving_restart==nrestart)then
                          x=(xg(ind_grid(i),1)+xc(ind,1)-skip_loc(1))*scale
                          y=(xg(ind_grid(i),2)+xc(ind,2)-skip_loc(2))*scale
                          z=(xg(ind_grid(i),3)+xc(ind,3)-skip_loc(3))*scale
                          radius = (x-30.)**2 + (y-30.)**2 + (z-30.)**2
                          if (radius > carving_radius**2) then
                            !xx(i)=xx(i) * ( 0.99 * (radius/15d0)**2 - 3.96 * (radius/15d0) + 3.97 ) 
                            xx(i)=xx(i) / 1e4
                          endif
                     endif

...
!################################################################
#if NDIM==3
subroutine thermal_feedback(ilevel, dt)
  use pm_commons
  use amr_commons
  use hydro_commons
  implicit none
  ...
  integer,dimension(1:nvector),save::ind_gridp,ind_part,ind_grid_part
  ...
  do icpu=1,ncpu
     ...
     do jgrid=1,numbl(icpu,ilevel)
        ...
        if(npart2>0)then
           ...
           do jpart=1,npart1
              ...
              if(ip==nvector)then
                 call feedbk(ind_gridp,ind_part,ind_grid_part,ig,ip,ilevel,dt)
                 ip=0
                 ig=0
              end if
              ipart=next_part  ! Go to next particle
     ...
     ! End loop over grids
     if(ip>0)call feedbk(ind_gridp,ind_part,ind_grid_part,ig,ip,ilevel,dt)
  end do
  ! End loop over cpus
...
!################################################################
#if NDIM==3
subroutine feedbk(ind_gridp,ind_part,ind_grid_part,ng,np,ilevel,dt)
  use amr_commons
  use pm_commons
  use hydro_commons
  use rt_parameters,only:SED_DIR
  use amr_parameters,only:energy_sn,m_remnant
  use random
  implicit none
  integer::ng,np,ilevel
  integer,dimension(1:nvector)::ind_gridp
  ...
  ! unew.
  !-----------------------------------------------------------------------
  integer::i,j,jj,idim,nx_loc,ivar,ilun=0
  real(kind=8)::RandNum
  ...
  real(dp),dimension(1:nvector),save::mloss,mzloss,ethermal,ekinetic,dteff,p_wind
  real(dp),dimension(1:nvector),save::vol_loc
  real(dp),dimension(1:nvector,1:ndim),save::x,pxx
  integer ,dimension(1:nvector,1:ndim),save::id,igd,icd
  integer ,dimension(1:nvector),save::igridp,icell,indp,kg,pcell,pgrid,pind
  real(dp),dimension(1:ndim)::neighxx
  integer::neighcell,neighgrid,neighind

  real(dp)::dt, r, distance, factor, star_age, t_sn
  integer::igrid,ncache,iskip,ngrid
  ...
  if(.not.allocated(star_list)) then
   inquire(file=TRIM(sed_dir)//'/star_list.dat', exist=ok_file)
   if(ok_file)then
    open(unit=19,file=TRIM(sed_dir)//'/star_list.dat', status='old',form='formatted')
    nlines=0
    do
      read(19,*, END=128)
      nlines = nlines + 1
    ...
    allocate(star_list(nlines,4))
    open(19, file=trim(TRIM(sed_dir)//'/star_list.dat'), form='formatted')
    iline=1
    do
      read(19, *, iostat=ios) star_list(iline,:)
      if (ios /= 0) exit
      iline = iline + 1
    end do
    close(19)
  ...
  do idim=1,ndim
     do i=1,ng
        x0(i,idim)=xg(ind_gridp(i),idim)-3.0D0*dx
     end do
  end do

  ! Gather 27 neighboring father cells (should be present anytime !)
  do i=1,ng
     ind_cell(i)=father(ind_gridp(i))
  end do
  ...
  do idim=1,ndim
     do j=1,np
        x(j,idim)=xp(ind_part(j),idim)/scale+skip_loc(idim) ! particle position in internal units
     end do
  end do
  do idim=1,ndim
     do j=1,np
        x(j,idim)=x(j,idim)-x0(ind_grid_part(j),idim) ! position wrt grid corner
     end do
  end do
  do idim=1,ndim
     do j=1,np
        x(j,idim)=x(j,idim)/dx ! particle position wrt to grid, in cell units
     end do
  end do
  ...
  do idim=1,ndim
     do j=1,np
        id(j,idim)=int(x(j,idim)) ! convert x to int (id=2,3,4)
     end do
  end do
  ...
  do idim=1,ndim
     do j=1,np
        igd(j,idim)=id(j,idim)/2 ! where particle is in grid (igd=1,2)
     end do
  end do
  do j=1,np
  ...
  do j=1,np
     igridp(j)=son(nbors_father_cells(ind_grid_part(j),kg(j)))
  end do

  ! Check if particles are entirely in level ilevel
  ok(1:np)=.true.
  do j=1,np
     ok(j)=ok(j).and.igridp(j)>0
  end do
  ...
  do j=1,np
     if(ok(j))then
        indp(j)=ncoarse+(icell(j)-1)*ngridmax+igridp(j)
     else
  ...
  !indp(j)  -> indice della cella in cui si trova la stella j
  !nbors_father_cells[ind_grid_part(j), :] -> indici delle (27) neighboring cells
  ...
  do j=1,np
      pgrid(j) = mod((indp(j)-ncoarse), ngridmax)
    end do
  do j=1,np
    pind(j)  = (pcell(j)-pgrid(j)-ncoarse)/ngridmax + 1
  end do
  do j=1,np
    do idim=1,ndim
      pxx(j, idim) = xg(pgrid(j),idim)+xc(pind(j),idim)
    end do
  end do
  do j=1,np
    do idim=1,ndim
      pxx(j, idim) = (pxx(j,idim)-skip_loc(idim))*scale
    end do
  end do
  ...
  do j=1,np
   ...
   call getAgeGyr(tp(ind_part(j)), star_age)
   t_sn = star_list(idp(ind_part(j)), 4) / 1.e3    ! SN time in Gyr

   if(star_age < t_sn) then

    ethermal(j) = 0d0
    mloss(j) = star_list(idp(ind_part(j)), 2) * dtnew(levelp(ind_part(j))) !total mass injected
    ekinetic(j)  = star_list(idp(ind_part(j)), 3) / star_list(idp(ind_part(j)), 2) !energy per unit mass
    p_wind(j) = (2*ekinetic(j))**0.5*mloss(j)

    ! reduce particle mass
    mp(ind_part(j)) = mp(ind_part(j)) - mloss(j)

    do jj=1,threetondim
      do neighind=1,twotondim
        neighgrid = son(nbors_father_cells(ind_grid_part(j),jj))
        neighcell = neighgrid + ncoarse + (neighind -1)*ngridmax
      do idim=1,ndim
        neighxx(idim) = xg(neighgrid,idim)+xc(neighind,idim)
        neighxx(idim) = (neighxx(idim)-skip_loc(idim))*scale
      end do

      r = max(abs(neighxx(1) - pxx(j,1)), abs(neighxx(2) - pxx(j,2)), abs(neighxx(3) - pxx(j,3))) / dx_loc
      if (r < 1.1) then
        if (neighcell == pcell(j)) then

          factor = 1./27

          do ivar=ndim+3+nener, nvar
            unew(neighcell, ivar)=unew(neighcell,ivar) + uold(neighcell,ivar)/uold(neighcell,1)*factor*mloss(j)/vol_loc(j)
          end do
          unew(neighcell,1)=unew(neighcell,1)+SUM(uold(neighcell,ndim+3+nener:nvar)/uold(neighcell,1)*factor*mloss(j)/vol_loc(j))
          unew(neighcell,5)=unew(neighcell,5)+factor*mloss(j)/vol_loc(j)*ekinetic(j)+ &
                        & ethermal(j)*(1d0+RAD_BOOST)

        else

           ! Update hydro variable in NGP cell
           distance =  (neighxx(1) - pxx(j,1))**2 + (neighxx(2) - pxx(j,2))**2 + (neighxx(3) - pxx(j,3))**2
           distance = SQRT(distance)
           factor   = 26./27 * 3./44 * (dx_loc/distance)**2

            do ivar=ndim+3+nener, nvar
              unew(neighcell, ivar)=unew(neighcell,ivar) + uold(neighcell,ivar)/uold(neighcell,1)*factor*mloss(j)/vol_loc(j)
            end do
            unew(neighcell,1)=unew(neighcell,1)+SUM(uold(neighcell,ndim+3+nener:nvar)/uold(neighcell,1)*factor*mloss(j)/vol_loc(j))
            unew(neighcell,2)=unew(neighcell,2)+factor*p_wind(j)/vol_loc(j)*(neighxx(1) - pxx(j,1))/distance
            unew(neighcell,3)=unew(neighcell,3)+factor*p_wind(j)/vol_loc(j)*(neighxx(2) - pxx(j,2))/distance
            unew(neighcell,4)=unew(neighcell,4)+factor*p_wind(j)/vol_loc(j)*(neighxx(3) - pxx(j,3))/distance
            unew(neighcell,5)=unew(neighcell,5)+factor*mloss(j)/vol_loc(j)*ekinetic(j)+ &
                          & ethermal(j)*(1d0+RAD_BOOST)
  ...
  else ! if star_age > t_sn

    if(mp(ind_part(j)) < 8.*2.e33 / scale_d / scale_l**3 ) cycle

    print*, '====== SUPERNOVA EXPLOSION ======='

    ethermal(j) = 0d0
    mloss(j) = mp(ind_part(j)) - m_remnant*2.e33/scale_d/scale_l**3   !total mass injected
    ekinetic(j)  = energy_sn/scale_d/scale_l**3/scale_v**2 / mloss(j)  !energy per unit mass
    p_wind(j) = (2*ekinetic(j))**0.5*mloss(j)

    ! reduce particle mass
    mp(ind_part(j)) = m_remnant*2.e33/scale_d/scale_l**3
...
!################################################################
!################################################################
! subroutine feedback_wind(xx,uu,dx,dt,nn,np,pxx)
!   use amr_commons
!   use hydro_parameters
!   implicit none
!   integer ::nn,np
!   real(dp)::dx,dt,dx_cgs,dt_cgs
!   integer ,dimension(1:nvector,1:ndim)::pxx
!   real(dp),dimension(1:nvector,1:nvar)::uu
!   real(dp),dimension(1:nvector,1:ndim)::xx
!   integer::i,j,group_ind
...
!   do j=1,np
!
!         print*, 'particle cell', ppx(j,:)
!
!         do i=1,nn
!            ! Compute position in normalized coordinates
!            xn=0.0d0; yn=0.0d0; zn=0.0d0
!            xn=2.0d0*abs(xx(i,1)-xx(indp(j), 1)) / (2.1*dx)
! #if NDIM>1
!            yn=2.0d0*abs(xx(i,2)-xx(indp(j), 2)) / (2.1*dx)
! #endif
! #if NDIM>2
!            zn=2.0d0*abs(xx(i,3)-xx(indp(j), 3)) / (2.1*dx)
! #endif

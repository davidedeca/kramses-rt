subroutine init_part
  ...
  character(LEN=1000)::filename,filename_x
    ...
    if(TRIM(initfile(levelmin)).NE.' ')then
       ...
       do while (.not.eof)
          ...
          if(myid==1)then
             jpart=0
             do i=1,nvector
                read(10,*,end=100)mm1,xx1,xx2,xx3,vv1,vv2,vv3,ll1,ll2,ll3 !,FAM_PART
                jpart=jpart+1
                indglob=indglob+1
...
100          continue
    ...
    do ipart =1, npartmax
        if (xp(ipart,3) == 0)xp(ipart,3)=10000000
    enddo

...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  !-------------------------------------------------------------------
  !-------------------------------------------------------------------
  integer::i,ind,iskip,idim,nleaf,nx_loc,ij
  ...
  real(dp),parameter::z_sun_par           = 0.02
  ...
  do ind=1,twotondim
     ...
     do i=1,nleaf
        ...
        !do i_krome =1,krome_nmols
        !    if (uold(ind_leaf(i),krome_id_vec(i_krome))==0) print*, 'before rt_solve_cooling', uold(ind_leaf(i),krome_id_vec(i_krome)), krome_id_vec(i_krome)
        !enddo 
     ...
     do i=1,nleaf
        ...
        !do ij=ndim+3+nener, nvar
        !    uold(ind_leaf(i), ij) = max(uold(ind_leaf(i),ij),smallr)
        !end do
     end do
...
#if defined(krome_on) 
        ! converti direttamente qui
        ! e recupera 1/mu alla temperatura
        do i=1,nleaf
     ...
     do i=1,nleaf
        uold(ind_leaf(i),ndim+2) = T2(i)+ekk(i)+err(i)
     end do
#else
     ...
     if(neq_chem) then
        ...
        do i_krome =1,krome_nmols
           do i=1,nleaf
              ...
              !if (uold(ind_leaf(i),krome_id_vec(i_krome))==0.0) then
              !  print*, 'after cooling', uold(ind_leaf(i), krome_id_vec(i_krome)), krome_id_vec(i_krome)
              !endif
           end do
        enddo
#else
